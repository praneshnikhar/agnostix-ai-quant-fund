...
def _bar(**kw: object) -> Bar:
    base: dict[str, object] = dict(
        symbol="AAPL", event_time=T0, open=100.0, high=110.0,
        low=99.0, close=105.0, volume=1000.0, provider_info=PROV, received_at=RECV,
    )
    base.update(kw)
...
def test_quote_crossed_book() -> None:
    q = Quote(symbol="AAPL", event_time=T0, bid_price=101.0, ask_price=100.0,
              provider_info=PROV, received_at=RECV)
...
def test_trade_nonpositive() -> None:
    ...
    t = Trade.model_construct(symbol="AAPL", event_time=T0, price=-1.0, size=10.0,
                              provider_info=PROV, received_at=RECV)
    assert any("price" in p for p in validate_trade(t))


def test_news_bad_url_scheme() -> None:
    n = NewsArticle(provider_article_id="1", headline="h", url="ftp://x",
                    published_at=T0, provider_info=PROV, received_at=RECV)
